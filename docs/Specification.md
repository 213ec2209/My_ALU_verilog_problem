...
On every positive edge of clk, the ALU samples inputs A, B, and op
...
The zero flag Z is asserted if the result equals zero
...
The ALU module accepts two 8-bit input operands (A and B) and a 3-bit operation selector (op). Based on the value of op, and rising edge of clock the ALU computes the corresponding arithmetic or logical operation and produces an 8-bit output (Y). Additionally, a zero flag (Z) is asserted when the output result equals zero.
...
Each rising edge of the clock triggers one complete ALU operation
